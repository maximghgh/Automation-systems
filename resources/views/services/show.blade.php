...
<body>@include('components.header')
  ...
  @php
    ...
    $contentSource = trim((string) $service->content) !== ''
    ...
    $normalizeContentHtml = static function (string $content): string {
      ...
      $wrappedHtml = '<?xml encoding="utf-8" ?><div id="service-content-root">' . $html . '</div>';
      ...
      $root = $xpath->query('//*[@id="service-content-root"]')->item(0);
  ...
  <main class="page">
    <div class="card-project">
      <div class="card__links">
        <a class="card__link" href="{{ route('services.index') }}">&#1059;&#1089;&#1083;&#1091;&#1075;&#1080;</a>
        ...
        <a class="card__link active" href="{{ route('services.show', $service) }}">{{ $service->title }}</a>
      </div>

      <div class="card-project__content">
        <div class="card-project__image">
          <a href="{{ $mediaUrl($service->image, '/assets/6d77fe7c4a76248ddd7c01fd7a810ac36f81f17e.png') }}" target="_blank" rel="noopener noreferrer" class="card__image-link">
            <img src="{{ $mediaUrl($service->image, '/assets/6d77fe7c4a76248ddd7c01fd7a810ac36f81f17e.png') }}" alt="{{ $service->title }}">
          </a>
        </div>
        <p class="card-project__name">{{ $service->title }}</p>
